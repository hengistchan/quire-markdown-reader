...
- **On-device preferences and handoffs:** Reader settings, onboarding state, up to six recent-document titles and URLs, and a browser-managed reference to the most recent folder are stored on the user's device. A document imported from an active page or local address is placed in the extension's local IndexedDB under a random, single-use handoff ID. The matching reader consumes and deletes only that handoff.
...
- `storage`: keep reader preferences, recent-document metadata, and onboarding state on the device. Document handoffs use the browser's local IndexedDB and do not require the `storage` permission.
...
Settings and recent metadata remain in browser storage until the user clears extension data or uninstalls Quire. Temporary active-page and local-address handoffs are deleted when the matching reader consumes them. An abandoned handoff expires after ten minutes and is removed during the next extension startup or handoff cleanup. Folder and file-URL access can be revoked through browser extension settings. Remote-origin permissions can be removed through the browser's extension permissions page.
