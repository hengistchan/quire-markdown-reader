        ...
        node.kind === 'directory' ? (
          <div key={node.id} className="tree-group">
            <button
              ...
              style={{ paddingInlineStart: `${12 + node.depth * 13}px` }}
          ...
          <button
            key={node.id}
            className={`file-row ${activeId === node.id ? 'active' : ''}`}
            style={{ paddingInlineStart: `${18 + node.depth * 13}px` }}
            onClick={() => onOpen(node.file)}
          >
            <File />
            <span>{node.name}</span>
            {activeId === node.id && <Check className="row-check" />}
          </button>
        ),
      )}
    </>
