...
export function CommandPalette({
    ...
    mode === 'recent'
      ...
      : [
          ...
          },
          {
            ...
            detail: 'HTTP / HTTPS',
            icon: <Globe2 />,
            shortcut: shortcuts.openUrl,
            run: onUrl,
          },
          {
            key: 'workspace',
            label: t('toggleWorkspace'),
            detail: t('files'),
            icon: <Folder />,
            shortcut: '',
            run: onWorkspace,
  ...
  const hasResults =
    actions.length || visibleRecent.length || visibleMatches.length || visibleWorkspaceMatches.length || typedRemoteUrl;
  ...
  return (
    <div ref={modalRef} className="command-backdrop" tabIndex={-1} onMouseDown={onClose}>
      <section
      ...
      >
        <div className="command-input">
          ...
          <input
            ...
            placeholder={mode === 'recent' ? t('searchRecentlyOpened') : t('commandPlaceholder')}
          />
          <kbd>esc</kbd>
        </div>
        <div className="command-results">
          {typedRemoteUrl && (
            <div className="command-group">
              <label>URL</label>
              <button className="command-row active" onClick={() => onTypedUrl(query.trim())}>
                <Globe2 />
                <span>
                  <strong>{t('openUrl')}</strong>
          ...
          {visibleRecent.length > 0 && (
            <div className="command-group">
              ...
              {visibleRecent.map((item, index) => (
                <button
                  key={item.id}
                  className={`command-row ${!needle && index === 0 ? 'active' : ''}`}
                  onClick={() => onRecent(item)}
                >
                  <RecentResourceIcon resource={item} />
                  <span>
                    <strong>{item.title}</strong>
                    <small>{recentResourceDetail(item, t)}</small>
                  </span>
                </button>
              ))}
            </div>
          )}
          {visibleWorkspaceMatches.length > 0 && (
            <div className="command-group">
              <label>{t('workspaceFiles')}</label>
              {visibleWorkspaceMatches.map((file) => (
                <button key={file.id} className="command-row workspace-match" onClick={() => onWorkspaceFile(file)}>
                  <File />
                  <span>
            ...
            <div className="command-group">
              ...
              {actions.map((action) => (
                <button
                  ...
                  onClick={() => {
                    action.run();
                    if (action.key !== 'url' && action.key !== 'settings') onClose();
                  }}
                >
                  {action.icon}
                  <span>
          ...
          {visibleMatches.length > 0 && (
            <div className="command-group">
              <label>{t('currentDocument')}</label>
              {visibleMatches.map((match) => (
                <button key={match.id} className="command-row document-match" onClick={() => onMatch(match)}>
                  <Search />
                  <span>
                    <strong>{match.text}</strong>
                    <small>
                      {t('line')} {match.lineNumber}
                    </small>
                  </span>
                </button>
              ))}
