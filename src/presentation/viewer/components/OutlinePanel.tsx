...
export function OutlinePanel({
  ...
  return (
    <>
      <nav className="outline-navigation" ref={navRef}>
        {headings.map((heading) => (
          <button
            key={heading.id}
            className={activeId === heading.id ? 'active' : ''}
            style={{ paddingInlineStart: `${10 + Math.max(0, heading.level - 1) * 8}px` }}
            onClick={() => onJump(heading.id)}
          >
            {heading.text}
          </button>
        ))}
        {headings.length === 0 && <p className="outline-empty">{t('noOutline')}</p>}
      </nav>
      <div className="outline-progress">
        <span>
          {t('readingProgress')} {Math.round(progress)}%
        </span>
        <i>
