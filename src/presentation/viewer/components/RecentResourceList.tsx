...
export function RecentResourceList({
  items,
  t,
  onOpen,
  onRemove,
  ...
  return (
    <section className="open-menu-section recent-resource-section" aria-label={t('recentlyOpened')}>
      <label>{t('recentlyOpened')}</label>
      {items.slice(0, OPEN_MENU_RECENT_LIMIT).map((resource) => (
        <RecentResourceRow key={resource.id} resource={resource} t={t} onOpen={onOpen} onRemove={onRemove} />
      ))}
      <button className="view-all-recent" data-open-menu-primary="true" onClick={onViewAll}>
