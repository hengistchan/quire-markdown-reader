...
export function useDocumentOpen(options: DocumentOpenOptions) {
  ...
  const openImported = useCallback(
    async (
      ...
      if (navigationMode !== 'traverse')
        navigation.record(
          {
            document: { kind: 'imported', sessionId: opened.sessionId },
            fragment,
          },
          navigationMode,
        );
  ...
  const openWorkspaceFile = useCallback(
    async (
      ...
      if (
        targetWorkspace.transient &&
        navigationMode === 'push' &&
        embeddedLocalSourceUrl.current &&
        window.top !== window
      ) {
  ...
  const openLocalHandle = useCallback(
    async (
      ...
      if (navigationMode !== 'traverse')
        navigation.record(
          {
            document: { kind: 'local-file', fileId },
            fragment,
          },
          navigationMode,
        );
  ...
  const openDroppedFile = useCallback(
    async (file: File, operationSignal?: AbortSignal) => {
      const signal = operationSignal ?? navigationOperation.begin();
      if (signal.aborted) return;
    ...
    async (
      ...
      try {
        ...
        if (navigationMode !== 'traverse')
          navigation.record(
            {
              document: { kind: 'remote', url: documentUrl.href },
              fragment,
            },
            navigationMode,
          );
