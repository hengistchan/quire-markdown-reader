...
import type { ImportedDocument, SidebarMode, WorkspaceFile, WorkspaceSnapshot } from '../../../shared/types';
...
export function useNavigationRestoration(options: NavigationRestorationOptions) {
  ...
  const showFailure = useCallback(
    (failure: ReaderError, restore?: RestorableNavigation) => {
  ...
  const navigateToTarget = useCallback(
    async (target?: NavigationTarget, operationSignal?: AbortSignal) => {
      ...
      if (!target) {
        options.replaceSession(createWelcomeSession(options.t('welcomeDocumentTitle'), options.t('welcomeDocument')));
      ...
      try {
        if (target.document.kind === 'workspace-file') {
          ...
          if (permission !== 'granted') {
            ...
            showFailure({ code: 'permission-required', retryable: true }, { target, kind: 'workspace' });
            return;
          }
        ...
        if (target.document.kind === 'local-file') {
          ...
          if (signal.aborted) return;
          if (permission !== 'granted') {
            showFailure({ code: 'permission-required', retryable: true }, { target, kind: 'local-file' });
            return;
          }
        ...
        if (target.document.kind === 'remote') {
          ...
          if (signal.aborted) return;
          if (!hasPermission) {
            showFailure({ code: 'permission-required', retryable: true }, { target, kind: 'remote' });
            return;
          }
