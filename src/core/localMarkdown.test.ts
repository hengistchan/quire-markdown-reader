...
import {
  ...
  isSelectLocalMarkdownWorkspaceFileMessage,
  localMarkdownPathWithinDirectory,
  localMarkdownTitle,
  localMarkdownWorkspaceFileUrl,
  localMarkdownWorkspaceHash,
  localMarkdownWorkspaceRoute,
  NAVIGATE_LOCAL_MARKDOWN_WORKSPACE,
  OPEN_LOCAL_MARKDOWN,
  READ_LOCAL_MARKDOWN_ASSET,
  SELECT_LOCAL_MARKDOWN_WORKSPACE_FILE,
} from './localMarkdown';

describe('local Markdown address handling', () => {
  it.each(['file:///Users/example/README.md', 'file:///tmp/guide.markdown', 'file:///C:/docs/NOTES.MDX'])(
    'accepts a local Markdown URL: %s',
    (value) => {
      expect(isLocalMarkdownUrl(value)).toBe(true);
    },
  );

  it.each(['file:///tmp/photo.png', 'https://example.com/readme.md', 'not a url'])(
    'ignores non-local Markdown input: %s',
    (value) => {
  ...
  it('resolves iframe workspace routes without crossing the selected local folder', () => {
    expect(
      localMarkdownWorkspaceFileUrl('file:///Users/example/work/mihomo/README.md', 'mihomo', 'docs/guide.md'),
    ).toBe('file:///Users/example/work/mihomo/docs/guide.md');
    expect(
      localMarkdownWorkspaceFileUrl('file:///Users/example/work/mihomo/docs/current.md', 'mihomo', 'notes/设计.md'),
    ).toBe('file:///Users/example/work/mihomo/notes/%E8%AE%BE%E8%AE%A1.md');
    expect(
      localMarkdownWorkspaceFileUrl('file:///Users/example/work/mihomo/README.md', 'mihomo', '../outside.md'),
    ).toBeUndefined();
    expect(
      localMarkdownWorkspaceFileUrl('file:///Users/example/work/mihomo/README.md', 'other', 'guide.md'),
    ).toBeUndefined();
  });
  ...
  it('round-trips safe workspace state through the top-page hash route', () => {
    ...
    expect(
      localMarkdownWorkspaceRoute('file:///tmp/README.md#quire-workspace=docs&quire-file=../private.md'),
    ).toBeUndefined();
  });
