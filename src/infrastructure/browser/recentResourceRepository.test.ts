...
import {
  loadRecentResources, MAX_RECENT_RESOURCES, rememberRecentResource,
  removeRecentResource, updateRecentWorkspaceDocument,
...
describe('recent resources repository', () => {
  ...
  it('sorts mixed resources by explicit-open time when loading', async () => {
    installStorage({ version: 1, items: [
    ...
    await expect(loadRecentResources()).resolves.toMatchObject([
      { title: 'B' },
      { title: 'C' },
      { title: 'A' },
    ]);
  ...
  it('updates a workspace last document without changing openedAt or order', async () => {
    const { set } = installStorage({ version: 1, items: [
    ...
    expect(result).toEqual([
      { id: 'workspace:a', title: 'A', kind: 'workspace', workspaceId: 'a', lastFilePath: 'docs/design.md', openedAt: 9 },
      { id: 'workspace:b', title: 'B', kind: 'workspace', workspaceId: 'b', openedAt: 5 },
    ]);
  ...
  it('removes only the selected resource', async () => {
    ...
    await expect(removeRecentResource('local-file:a')).resolves.toMatchObject([
      { id: 'local-file:b' },
    ]);
  });
  ...
  it('treats malformed or unavailable storage as empty and ignores write failures', async () => {
    ...
    vi.stubGlobal('browser', { storage: { local: {
      ...
      set: vi.fn(async () => { throw new Error('quota exceeded'); }),
    } } });
    await expect(rememberRecentResource({
