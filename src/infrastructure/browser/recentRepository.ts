import type { RecentItem, RecentItemInput, RecentRepository } from '../../application/ports/recentRepository';
...
function normalizeItem(value: unknown): RecentItem | undefined {
  ...
  const scrollPosition =
    typeof value.scrollPosition === 'number' && Number.isFinite(value.scrollPosition) && value.scrollPosition >= 0
      ? value.scrollPosition
      : undefined;
  const headingId = typeof value.headingId === 'string' ? value.headingId : undefined;
  if (value.kind === 'remote' && typeof value.url === 'string')
    return { ...base, kind: 'remote', url: value.url, scrollPosition, headingId };
  ...
  if (value.kind === 'local-file' && typeof value.fileId === 'string')
    return { ...base, kind: 'local-file', fileId: value.fileId, scrollPosition, headingId };
  return undefined;
}
...
export async function loadRecentItems(): Promise<RecentItem[]> {
  ...
  try {
    await browser.storage.local.set({
      [STORAGE_KEY]: { version: SCHEMA_VERSION, items: migrated } satisfies PersistedRecentItems,
    });
  } catch {
...
export async function rememberRecentItem(item: RecentItemInput): Promise<RecentItem[]> {
  ...
  const next = [
    { ...previous, ...item, openedAt: Date.now() } as RecentItem,
    ...current.filter((entry) => entry.id !== item.id),
  ].slice(0, MAX_RECENT);
  if (typeof browser !== 'undefined' && browser.storage) {
    await browser.storage.local.set({
      [STORAGE_KEY]: { version: SCHEMA_VERSION, items: next } satisfies PersistedRecentItems,
    });
  }
  return next;
}

export async function updateRecentPosition(
  id: string,
