...
describe('DocumentSession', () => {
  ...
  it('applies refresh events only to compatible session kinds', () => {
    ...
    expect(
      documentSessionReducer(welcome, {
        type: 'refresh-remote',
        document: { title: 'Wrong.md', markdown: '# Wrong' },
        state: { url: 'https://example.com' },
      }),
    ).toBe(welcome);
    expect(
      documentSessionReducer(welcome, {
        type: 'localize-welcome',
        title: '欢迎',
        markdown: '# 欢迎',
      }),
  ...
  it('updates remote content and validators atomically', () => {
    const remote = createRemoteSession(
      { title: 'Old.md', markdown: '# Old' },
      { url: 'https://example.com/doc.md', etag: 'v1' },
    );
    expect(
      documentSessionReducer(remote, {
        type: 'refresh-remote',
