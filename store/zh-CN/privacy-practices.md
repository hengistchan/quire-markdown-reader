...
政策核对日期：2026-08-02。Chrome Web Store 自 2026-08-01 起要求所有数据处理都向用户醒目披露；中英文商店说明已在“单一用途”之后加入“数据处理醒目说明”，覆盖本地文档、当前页面快照、网络请求、最近记录、阅读位置和文件句柄。

## 单一用途说明

Quire 的单一用途是把用户明确选择的 Markdown 内容呈现为专注、只读的浏览器阅读工作区。内容来源包括用户选择的本地 .md、.markdown 或 .mdx 文件及文件夹、用户在地址栏明确打开的本地 Markdown 绝对路径、用户输入的远程 Markdown 网址，以及用户通过工具栏、快捷键或右键菜单主动导入的当前页面可见文本。当前页面内容作为纯文本快照显示；.mdx 按普通 Markdown 安全预览，其中的 JSX、导入语句和表达式不会执行。扩展仅为阅读目的在本机处理内容，并提供文件树、大纲、搜索、排版、代码高亮、KaTeX 和 Mermaid 等直接相关功能。Quire 不编辑源文件，不在后台读取浏览历史，也不提供与 Markdown 阅读无关的功能。

## 需请求 activeTab 的理由

activeTab 仅在用户明确点击 Quire 工具栏按钮、执行 Alt/Option + Shift + M 快捷键或选择“使用 Quire 打开”右键菜单时，临时授权访问当时的活动标签页。Quire 使用该临时访问读取页面标题、网址和可见文本，并在本机阅读器中作为纯文本快照显示；不会把它当作 Markdown 或代码执行，不会持续访问其他标签页，不会在后台收集浏览记录，用户离开或关闭该页面后临时访问即失效。
...
storage 用于在用户设备上的 chrome.storage.local 中保存阅读设置、语言和主题、首次使用状态，以及最多六条最近文档的标题、来源引用和阅读位置。Quire 不使用 storage 保存长期文档、上传或同步内容，也不向开发者或第三方传输这些数据；用户可通过清除扩展数据或卸载扩展删除它们。为让用户重新打开不同的本地文件和工作区，浏览器管理的文件或文件夹句柄会按随机 ID 保存在本机 IndexedDB，浏览器要求时仍会重新请求权限。网页或本地地址导入使用另一套带随机 ID 的一次性交接；对应阅读器读取后立即删除，未读取交接在十分钟后过期，并在下次扩展启动或交接清理时移除。
