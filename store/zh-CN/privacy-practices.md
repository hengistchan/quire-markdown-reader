...
Quire 的单一用途是把用户明确选择的 Markdown 内容呈现为专注、只读的浏览器阅读工作区。内容来源包括用户选择的本地 .md、.markdown 或 .mdx 文件及文件夹、用户输入的远程 Markdown 网址，以及用户通过工具栏、快捷键或右键菜单主动导入的当前页面可见文本。扩展仅为阅读目的在本机解析和渲染内容，并提供文件树、大纲、搜索、排版、代码高亮、KaTeX 和 Mermaid 等直接相关功能。Quire 不编辑源文件，不在后台读取浏览历史，也不提供与 Markdown 阅读无关的功能。
...
storage 用于在用户设备上的 chrome.storage.local 中保存阅读设置、语言和主题、首次使用状态、最多六条最近文档的标题及用户选择的网址，以及从当前页面导入后供阅读器一次性读取的临时文档。临时导入内容在阅读器载入后立即删除。Quire 不使用 storage 上传或同步文档，不向开发者或第三方传输这些数据；用户可通过清除扩展数据或卸载扩展删除它们。

## 可选主机权限说明

可选的 http://*/* 和 https://*/* 主机权限仅用于读取用户主动输入的远程 Markdown 网址。Quire 会在运行时只请求该网址所属来源的权限，并仅获取用户选择的文档，不会预先获得所有网站访问权，也不会在后台抓取网页。file://*/* 仅允许用户在 Chrome 扩展详情页主动开启“允许访问文件网址”后，从当前本地页面导入可见文本；通过文件选择器打开本地 Markdown 不依赖该权限。
